...
export const CatchRegistrationView: React.FC<Props> = ({
  ...
  const [selectedBoatReg, setSelectedBoatReg] = useState(
    availableBoats[0]?.registrationNumber || "KV-084-KSM",
  );
  const [selectedSpecies, setSelectedSpecies] =
    useState<SpeciesType>("NILE_PERCH");
  ...
  const [harvestMethod, setHarvestMethod] = useState(
    "Certified Gillnet (6-inch mesh)",
  );
  const [temperatureCelsius, setTemperatureCelsius] = useState<number>(3.2);
  const [iceRatio, setIceRatio] = useState<"1:1" | "1:2" | "1:3" | "NO_ICE">(
    "1:1",
  );
  const [iceSource, setIceSource] = useState(
    `${currentSite.name} Solar Flake Ice Plant`,
  );
  ...
  const activeBoat =
    availableBoats.find((b) => b.registrationNumber === selectedBoatReg) ||
    availableBoats[0];
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-teal-100/90 mt-1 max-w-xl">
              ...
              • BMU Leader: {currentSite.bmuLeader} •{" "}
              {currentSite.hasSolarIcePlant
                ? "☀️ Solar Ice Facility Active (0-3°C)"
                : "Standard Ice Storage"}
            </p>
          </div>
          ...
          <div className="bg-black/20 backdrop-blur-xs border border-white/15 rounded-lg p-3 flex items-center space-x-4">
            <div className="text-center">
              <div className="text-xl font-bold text-white font-mono">
                {
                  batches.filter((b) => b.landingSiteId === selectedSiteId)
                    .length
                }
              </div>
              <div className="text-[9px] uppercase font-bold text-teal-200 tracking-wider">
                Batches Today
              </div>
            </div>
        ...
        <div className="p-4 bg-emerald-950/90 border border-emerald-700/80 rounded-xl text-emerald-100 flex items-center justify-between shadow-xs">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="font-bold text-xs">
                Catch Successfully Recorded! Batch Code:{" "}
                <span className="font-mono text-white text-sm">
                  {submitSuccessMsg.batchId}
                </span>
              </div>
              <p className="text-[11px] text-emerald-300">
                Immutable cryptographic ledger event appended. SMS notification
                queued for boat captain.
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <button
              onClick={() => {
                const b =
                  batches.find((x) => x.batchId === submitSuccessMsg.batchId) ||
                  batches[0];
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                {(Object.keys(SPECIES_CATALOG) as SpeciesType[]).map(
                  (spKey) => {
                    ...
                    return (
                      ...
                      >
                        <div>
                          <div
                            className={`font-bold text-sm leading-tight ${isSelected ? "text-[#004D40]" : "text-slate-800"}`}
                          >
                            {sp.commonName}
                          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const b = availableBoats.find(
                      (item) => item.registrationNumber === e.target.value,
                    );
                ...
                >
                  {availableBoats.map((boat) => (
                    <option
                      key={boat.registrationNumber}
                      value={boat.registrationNumber}
                    >
                      {boat.registrationNumber} — {boat.name} (
                      {boat.captainName})
                    </option>
                  ))}
                </select>
                {activeBoat && (
                  <p className="text-[11px] text-slate-500 mt-1">
                    ...
                    <span className="font-semibold text-slate-700">
                      {activeBoat.captainName}
                    </span>{" "}
                    • Phone: {activeBoat.captainPhone}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                >
                  <option value="Certified Gillnet (6-inch mesh)">
                    Certified Gillnet (&gt;= 5-inch legal mesh)
                  </option>
                  <option value="Traditional Handline & Longline">
                    Traditional Handline &amp; Longline
                  </option>
                  <option value="Approved Hook & Line">
                    Approved Hook &amp; Line (Deep Water)
                  </option>
                  <option value="Purse Seine / Omena Ringnet (Certified Mesh)">
                    Certified Omena Ringnet (Dark moon phase)
                  </option>
                </select>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-1">
                  Est. value:{" "}
                  <span className="font-bold text-[#004D40] font-mono">
                    KES{" "}
                    {(
                      parseFloat(weightKg || "0") *
                      activeSpeciesInfo.indicativePricePerKgKes
                    ).toLocaleString()}
                  </span>
                </p>
              </div>
            ...
            <div className="p-4 bg-[#E0F2F1] rounded-xl border border-teal-200 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                <div>
                  ...
                  >
                    <option value="1:1">
                      1:1 Heavy Solar Flake Ice (Grade A)
                    </option>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTemperatureCelsius(parseFloat(e.target.value) || 0)
                    }
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-slate-100">
              ...
              >
                ...
                <span>
                  {isSubmitting
                    ? "Recording Catch..."
                    : "Register Batch & Generate QR Tag"}
                </span>
              </button>
            </div>
          </form>
            ...
            <div className="p-3 bg-slate-50 rounded-lg space-y-2 text-xs border border-slate-200">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-slate-900 font-mono">
                  KES {activeSpeciesInfo.indicativePricePerKgKes} / kg
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Max Shelf Life on Ice:</span>
                <span className="font-bold text-slate-900">
                  {activeSpeciesInfo.maxFreshHoursOnIce} hours
                </span>
              </div>
              {activeSpeciesInfo.minLegalLengthCm && (
                <div className="flex justify-between text-emerald-700 font-medium">
                  ...
                  <span className="font-bold">
                    &gt;= {activeSpeciesInfo.minLegalLengthCm} cm
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-xs text-slate-900 uppercase tracking-wider">
                Recent Batches Landed
              </h3>
              <span className="text-[10px] text-[#004D40] font-bold uppercase">
