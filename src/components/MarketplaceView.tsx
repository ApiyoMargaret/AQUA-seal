...
export const MarketplaceView: React.FC<Props> = ({
  ...
  const [selectedBatchForInquiry, setSelectedBatchForInquiry] =
    useState<FishBatch | null>(null);
  const [buyerName, setBuyerName] = useState("");
  const [buyerPhone, setBuyerPhone] = useState("+254 7");
  const [inquirySent, setInquirySent] = useState(false);

  const activeListings = batches.filter(
    (b) => b.listing?.isListed && b.status === "ACTIVE_LISTED",
  );

  const filteredListings = activeListings.filter((b) => {
    if (selectedSpecies !== "ALL" && b.species !== selectedSpecies)
      return false;
    return true;
  });
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold mt-1 tracking-tight">
              Fair-Trade Verified Micro-Marketplace
            </h1>
            <p className="text-xs text-teal-100/90 mt-1 max-w-xl leading-relaxed">
              Connect directly with artisanal fishers. Every batch comes with
              certified BMU landing provenance and solar cold-chain evidence.
              <strong className="text-teal-200">
                {" "}
                0% fisher registration fees • 1.5% transparent direct-sale buyer
                fee.
              </strong>
            </p>
          </div>
          ...
          <div className="bg-black/20 backdrop-blur-xs p-4 rounded-lg border border-white/15 text-xs space-y-1">
            <div className="font-bold text-teal-200">
              Lake Victoria Pricing Transparency
            </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          ...
          <div className="flex flex-wrap gap-1.5">
            {["ALL", "NILE_PERCH", "TILAPIA", "OMENA", "CATFISH"].map((sp) => (
              ...
              >
                {sp === "ALL"
                  ? "All Catches"
                  : SPECIES_CATALOG[sp as SpeciesType]?.commonName || sp}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="text-xs text-slate-500 font-medium">
          Showing{" "}
          <strong className="text-slate-900">{filteredListings.length}</strong>{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredListings.map((batch) => {
          const fees = calculateMarketplaceFees(
            batch.listing?.pricePerKgKes || 480,
            batch.currentWeightKg,
          );
          const speciesMeta =
          ...
          return (
            ...
            >
                ...
                <div className="pt-2 border-t border-slate-100 space-y-1 text-xs">
                  ...
                  <div className="flex justify-between text-slate-500">
                    ...
                    <span className="font-bold text-slate-800 font-mono">
                      KES {fees.grossTotalKes.toLocaleString()}
                    </span>
                  </div>
              ...
              <div className="p-4 bg-slate-50 border-t border-slate-200">
                <button
                  onClick={() => setSelectedBatchForInquiry(batch)}
                  className="w-full py-2.5 px-4 bg-[#006064] hover:bg-[#004D40] text-white font-bold text-xs uppercase tracking-wide rounded-lg shadow-xs transition-colors flex items-center justify-center space-x-2"
                >
                  <MessageSquare className="w-3.5 h-3.5" />
                  <span>Contact Seller (SMS Relay)</span>
                </button>
              </div>
            </div>
          );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 border border-slate-200 space-y-4">
            ...
            <p className="text-xs text-slate-500">
              Your inquiry will be relayed to boat captain{" "}
              <strong className="text-slate-800">
                {selectedBatchForInquiry.fisherName}
              </strong>{" "}
              via SMS relay without exposing private personal phone numbers.
            </p>
            ...
            {inquirySent ? (
              <div className="p-4 bg-emerald-50 text-emerald-900 rounded-lg text-xs font-semibold space-y-1">
                ...
                <div>
                  The seller and BMU desk have been notified of your offer.
                </div>
              </div>
            ) : (
              <form onSubmit={handleSendInquiry} className="space-y-3">
                ...
                <div className="p-3 bg-[#E0F2F1] rounded-lg border border-teal-200 text-[11px] text-[#004D40]">
                  Total Payable at Landing:{" "}
                  <strong className="font-mono">
                    KES{" "}
                    {selectedBatchForInquiry.listing?.estimatedTotalKes.toLocaleString()}
                  </strong>{" "}
                  (Includes 1.5% verified assurance fee).
                </div>
