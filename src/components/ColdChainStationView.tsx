      ...
      <div className="bg-[#004D40] rounded-xl p-6 border border-teal-900/60 shadow-xs text-white">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold mt-1 tracking-tight">Solar Ice &amp; Transport Hub</h1>
            <p className="text-xs text-teal-100/90 mt-1 max-w-xl">
              Fish spoilage in Lake Victoria occurs 4x faster without immediate 0-3°C icing.
              Record regular temperature audits, solar flake ice refills, and insulated transport seals.
            </p>
          </div>
          ...
          <div className="flex items-center space-x-3 bg-black/20 backdrop-blur-xs p-3 rounded-lg border border-white/15 text-xs">
            ...
            <div>
              <div className="font-bold text-white">Lake Victoria Cold Chain Rule</div>
              <div className="text-teal-200 text-[11px]">Core temp &lt; 4.0°C guarantees Grade A fresh seal</div>
            </div>
          </div>
        </div>
      </div>
          ...
          {selectedBatch && (
            <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-xs space-y-6">
              ...
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-wrap items-center justify-between gap-4">
                <div>
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Updating Batch</div>
                  <h3 className="text-xl font-bold font-mono text-slate-900">{selectedBatch.batchId}</h3>
                  <div className="text-xs text-slate-600">
                    {selectedBatch.species} • {selectedBatch.currentWeightKg} kg • {selectedBatch.boatName}
                  </div>
                </div>

                <div className="flex items-center space-x-3">
                  <div className="text-right">
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Current Grade</div>
              ...
              <form onSubmit={handleApplyColdChain} className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        >
                          <option value="1:1">1:1 Heavy Solar Flake Ice (0°C - 3°C)</option>
                          <option value="1:2">1:2 Standard Chilling (3°C - 6°C)</option>
                          <option value="1:3">1:3 Light Transport Crushed Ice</option>
                        </select>
                      </div>
                ...
                {actionTab === 'INSPECT' && (
                  <div className="space-y-4">
                    <div className="p-3.5 bg-[#E0F2F1] rounded-lg border border-teal-200 text-xs text-[#004D40]">
                      <strong>FAO Organoleptic Guidelines:</strong> Grade A fish requires clear convex eyes, bright red gills with no odor, and elastic flesh.
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                      <div>
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Eyes Clarity</label>
                        <select
                          value={sensoryEyes}
                          onChange={(e) => setSensoryEyes(e.target.value as any)}
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs text-slate-900 focus:ring-1 focus:ring-teal-700 focus:outline-hidden"
                        >
                          <option value="clear_bulging">Clear &amp; Convex / Bulging (Grade A)</option>
                          <option value="flat_slightly_cloudy">Flat &amp; Slightly Cloudy (Grade B)</option>
                          <option value="sunken_opaque">Sunken &amp; Opaque Gray (Unfit)</option>
                        </select>
                      </div>

                      <div>
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Gills Condition</label>
                        <select
                          value={sensoryGills}
                          onChange={(e) => setSensoryGills(e.target.value as any)}
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs text-slate-900 focus:ring-1 focus:ring-teal-700 focus:outline-hidden"
                        >
                          <option value="bright_red_mucus_free">Bright Red &amp; Mucus-Free (Grade A)</option>
                          <option value="pale_pink">Pale Pink / Light Mucus (Grade B)</option>
                          <option value="brown_sour_mucus">Brown / Sour Mucus (Unfit)</option>
                        </select>
                      </div>

                      <div>
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Flesh Elasticity</label>
                        <select
                          value={sensoryFlesh}
                          onChange={(e) => setSensoryFlesh(e.target.value as any)}
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs text-slate-900 focus:ring-1 focus:ring-teal-700 focus:outline-hidden"
                        >
                          <option value="firm_elastic">Firm &amp; Springy Elastic (Grade A)</option>
                          <option value="slightly_soft">Slightly Soft (Grade B)</option>
                          <option value="soft_dented">Soft / Finger Impression Remains (Grade C)</option>
                        </select>
                      </div>

                      <div>
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Odor / Smell</label>
                        <select
                          value={sensoryOdor}
                          onChange={(e) => setSensoryOdor(e.target.value as any)}
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs text-slate-900 focus:ring-1 focus:ring-teal-700 focus:outline-hidden"
                        >
                          <option value="fresh_seaweed_lake">Fresh Lake / Neutral Algae (Grade A)</option>
                          <option value="neutral_mild">Neutral Mild Fishy (Grade B)</option>
                          <option value="sour_stale">Sour / Ammoniacal (Unfit)</option>
                        </select>
                      </div>
                    </div>
                  </div>
                ...
                <div className="pt-3 border-t border-slate-200 flex justify-end">
                  ...
                  >
                    ...
                    <span>{isSubmitting ? 'Logging...' : 'Append Cold-Chain Record to Ledger'}</span>
                  </button>
                </div>
              </form>
            </div>
