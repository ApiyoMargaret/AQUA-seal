...
export const ConsumerVerificationView: React.FC<Props> = ({
  batches,
  initialBatchCode,
}) => {
  const [searchCode, setSearchCode] = useState(
    initialBatchCode || "LV-DG-20260821-042",
  );
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-[#004D40] rounded-xl p-8 border border-teal-900/60 shadow-xs text-white text-center space-y-4">
        ...
        <p className="text-xs text-teal-100/90 max-w-lg mx-auto leading-relaxed">
          Scan the QR code on your fish gill tag or enter the human-readable
          batch code to inspect origin, BMU landing time, solar icing evidence,
          and trust history.
        </p>
        ...
        <form
          ...
          className="max-w-md mx-auto flex items-center bg-white rounded-lg p-1 shadow-sm border border-teal-300"
        >
          <input
            type="text"
            value={searchCode}
      ...
      {verificationState === "LOADING" && (
        <div className="bg-white rounded-xl p-12 text-center border border-slate-200 shadow-xs space-y-3">
          ...
          <p className="text-xs font-semibold text-slate-600">
            Verifying cryptographic batch hashes...
          </p>
        </div>
      )}
        ...
        <div className="bg-white rounded-xl p-8 border border-rose-200 shadow-xs text-center space-y-4">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-900">
              Unverified Fish Code
            </h2>
            <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
              No authentic Lake Victoria BMU registration exists for code "
              <span className="font-mono font-bold text-rose-700">
                {searchCode}
              </span>
              ". This fish may be unregistered, illegally harvested, or
              counterfeit.
            </p>
          </div>
          <button
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden space-y-6 p-6 sm:p-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between pb-6 border-b border-slate-200 gap-4">
            ...
            <div className="p-3.5 bg-[#E0F2F1] rounded-xl border border-teal-200 text-center sm:text-right w-full sm:w-auto">
              ...
              <div className="text-xs text-teal-800 font-mono font-semibold mt-0.5 flex items-center justify-center sm:justify-end space-x-1">
                ...
                <span>
                  {verifiedData.currentTemperatureCelsius}°C • Score:{" "}
                  {verifiedData.freshnessScorePercent}%
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-3.5 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <div className="font-bold text-slate-900 text-xs mt-0.5 leading-tight">
                {verifiedData.speciesName}
              </div>
              <div className="text-[11px] text-slate-500 italic mt-0.5">
                {verifiedData.localSpeciesName}
              </div>
            </div>
            ...
            <div className="p-3.5 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <div className="font-bold text-slate-900 text-xs mt-0.5">
                {verifiedData.boatName}
              </div>
              <div className="text-[11px] text-slate-500 mt-0.5 truncate">
                {verifiedData.harvestMethod}
              </div>
            </div>
            ...
            <div className="p-3.5 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <div className="font-bold text-slate-900 text-xs mt-0.5">
                {new Date(verifiedData.harvestDate).toLocaleDateString(
                  "en-KE",
                  { month: "short", day: "numeric" },
                )}
              </div>
              <div className="text-[11px] text-slate-500 mt-0.5">
                {new Date(verifiedData.harvestDate).toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </div>
            ...
            <div className="p-3.5 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <div className="text-[11px] text-slate-500 mt-0.5">
                {verifiedData.trustReport.ledgerEventsCount} signed events
              </div>
            </div>
          </div>
              ...
              {verifiedData.timeline.map((step: any, idx: number) => (
                ...
                >
                  ...
                  {step.temperature !== undefined && (
                    <div className="text-[10px] text-[#006064] font-mono font-semibold">
          ...
          <div className="p-4 bg-[#E0F2F1] rounded-lg border border-teal-200 text-xs text-[#004D40] space-y-1.5">
            ...
            <p className="text-slate-700 leading-relaxed text-xs">
              This fish was registered directly at {verifiedData.landingSite}{" "}
              Beach Management Unit. Immediate solar flake icing has preserved
              cell freshness. All harvest methods comply with Kenya Marine and
              Fisheries Research Institute (KMFRI) mesh guidelines.
            </p>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 space-y-4 border border-slate-200">
            ...
            <p className="text-xs text-slate-500">
              Your feedback is audited by County Fisheries Officers and the
              Beach Management Unit.
            </p>

            {reportSubmitted ? (
              <div className="p-4 bg-emerald-50 text-emerald-900 rounded-lg text-xs font-semibold">
                Thank you! Your dispute report has been registered and forwarded
                to the BMU desk.
              </div>
            ) : (
              <form
