...
export default function App() {
  ...
  const [networkMode, setNetworkMode] = useState<'ONLINE' | 'INTERMITTENT_2G' | 'OFFLINE'>('ONLINE');
    ...
    <div className="min-h-screen bg-[#F1F5F9] text-slate-900 flex flex-col font-sans selection:bg-[#004D40] selection:text-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 space-y-3">
            ...
            <p className="text-sm font-semibold text-slate-600">Connecting to Aqua-Seal Lake Victoria Node...</p>
          </div>
        ) : (
          <>
      ...
      <footer className="bg-slate-900 border-t border-slate-800 text-slate-400 py-6 text-xs text-center space-y-1 mt-auto">
        <div className="font-semibold text-slate-300">
          Aqua-Seal Lake Victoria • Small-Scale Fisheries Provenance &amp; Micro-Marketplace
        </div>
        <p className="text-slate-500">
          Supporting Beach Management Units (BMUs) across Kisumu, Siaya, Homa Bay, Migori, and Busia. USSD Gateway: <span className="font-mono text-teal-400">*384*2782#</span> • SMS: <span className="font-mono text-teal-400">22384</span>
        </p>
      </footer>
